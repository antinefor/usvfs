...
public:
  ...
  void insertMapping(const HookContext::Ptr &context, bool directory = false)
  {
    ...
    else
    {
      ...
      m_FileNode =
        context->redirectionTable().addFile(m_RealPath, RedirectionDataLocal(shared::string_cast<std::string>(m_FileName, shared::CodePage::UTF8)));
  ...
  void removeMapping(const HookContext::ConstPtr &readContext, bool directory = false)
  {
    ...
    if (wasRerouted()) {
      if (m_FileNode.get())
        ...
        spdlog::get("usvfs")->warn("Node not removed: {}", shared::string_cast<std::string>(m_FileName));
        ...
        while (true)
        {
          ...
          if (k32FakeDirTracker.contains(parent))
          {
            ...
            if (RemoveDirectoryW(parent.c_str())) {
              ...
              spdlog::get("usvfs")->info("removed empty fake directory: {}", shared::string_cast<std::string>(parent));
            }
            else if (GetLastError() != ERROR_DIR_NOT_EMPTY) {
              auto error = GetLastError();
              spdlog::get("usvfs")->warn("removing fake directory failed: {}, error={}", shared::string_cast<std::string>(parent), error);
              break;
            }
          }
          else
  ...
  static bool addDirectoryMapping(const HookContext::Ptr &context, const fs::path& originalPath, const fs::path& reroutedPath)
  {
    if (originalPath.empty() || reroutedPath.empty()) {
      spdlog::get("hooks")->error("RerouteW::addDirectoryMapping failed: {}, {}",
        shared::string_cast<std::string>(originalPath.wstring(), shared::CodePage::UTF8).c_str(),
        shared::string_cast<std::string>(reroutedPath.wstring(), shared::CodePage::UTF8).c_str());
      return false;
    }
    ...
    std::string reroutedU8
      = shared::string_cast<std::string>(reroutedPath.wstring(), shared::CodePage::UTF8);
    if (reroutedU8.empty() || reroutedU8[reroutedU8.size() - 1] != '\\')
      reroutedU8 += "\\";

    spdlog::get("hooks")->info("mapping directory in vfs: {}, {}",
      shared::string_cast<std::string>(originalPath.wstring(), shared::CodePage::UTF8), reroutedU8.c_str());
    ...
    for (fs::directory_iterator itr(reroutedPath); itr != end_itr; ++itr)
    {
      ...
      if (is_regular_file(itr->path())) {
        std::string fileReroutedU8 = shared::string_cast<std::string>(itr->path().wstring(), shared::CodePage::UTF8);
        spdlog::get("hooks")->info("mapping file in vfs: {}, {}",
          shared::string_cast<std::string>((originalPath / itr->path().filename()).wstring(), shared::CodePage::UTF8),
  ...
  bool rerouteNew(const HookContext::ConstPtr &context, HookCallContext &callContext, LPCWSTR lpFileName, bool replaceExisting, const char* hookName)
  {
    ...
    if (!rerouteCreate(context, callContext, lpFileName, disposition, GENERIC_WRITE, nullptr)) {
      spdlog::get("hooks")->info(
        ...
        hookName, shared::string_cast<std::string>(lpFileName, shared::CodePage::UTF8), replaceExisting ? "true" : "false", error());
